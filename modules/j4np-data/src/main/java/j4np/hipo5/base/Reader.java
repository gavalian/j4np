...
public class Reader {
     ...
     *
     * @throws HipoException if bytes not in evio format
     */
    ...
    public static class RecordPosition {
        ...
        private long userWordOne = 0L;
        private long userWordTwo = 0L;
        private int  type;

        public RecordPosition(long pos) {position = pos;}
        public RecordPosition(long pos, int len, int cnt) {
            position = pos; length = len; count = cnt;
        }

        public RecordPosition setUserWordOne(long word){ userWordOne = word; return this;}
        public RecordPosition setUserWordTwo(long word){ userWordTwo = word; return this;}
        ...
        public long getUserWordOne(){ return this.userWordOne;}
        public long getUserWordTwo(){ return this.userWordTwo;}
        
        public static ByteBuffer build(List<RecordPosition> posList){
            int size = 3*8 + 2*4;
            byte[] data = new byte[posList.size()*size];
            ByteBuffer buffer = ByteBuffer.wrap(data);
            buffer.order(ByteOrder.LITTLE_ENDIAN);
            for(int i = 0; i < posList.size(); i++){
                RecordPosition pos = posList.get(i);
                int offset = i * size;
                buffer.putLong( offset +  0, pos.getPosition());
                buffer.putInt(  offset +  8, pos.getLength());
                buffer.putInt(  offset + 12, pos.getCount());
                buffer.putLong( offset + 16, pos.getUserWordOne());
                buffer.putLong( offset + 24, pos.getUserWordTwo());                
            }
            return buffer;
        }
        
        public static List<RecordPosition> buildList(ByteBuffer buffer){
            List<RecordPosition> posList = new ArrayList<RecordPosition>();
            int size = 3*8 + 2*4;
            int offset = 0;
            int limit  = buffer.capacity();
            while(offset + size < limit){
                RecordPosition pos = new RecordPosition(buffer.getLong(offset));
                pos.setLength( buffer.getInt(offset + 8));
                pos.setCount(  buffer.getInt(offset + 12));
                pos.setUserWordOne( buffer.getLong(offset+16));
                pos.setUserWordTwo( buffer.getLong(offset+24));
                posList.add(pos);
                offset += size;
            }
            return posList;
        ...
        public String toString(){
            return String.format(" POSITION = %16d, LENGTH = %12d, COUNT = %8d UID = %10d %10d", 
                    position, length, count, userWordOne,userWordTwo);
        }
    }
