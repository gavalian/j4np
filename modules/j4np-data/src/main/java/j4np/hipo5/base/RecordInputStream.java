...
public class RecordInputStream {
    ...
    public void copyEvent(ByteBuffer buffer, int boffset, int index) throws HipoException {
        ...
        I think getEvent method will work just fine*/
    }
    /**
    ...
    /**
     * returns the position of the event with index = "index" inside
     * the record buffer. 
     * @param index index of the event requested.
     * @return the offset position in the buffer.
     */
    public int getEventPosition(int index){
        if(index<0||index>=getEntries()) return -1;
        int firstPosition = dataBuffer.getInt((index - 1) * 4);
        int offset = eventsOffset + firstPosition;
        return offset;
    }
