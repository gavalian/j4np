...
public class EventFilter {
    ...
    int nChargedParticlesCount;
    int nChargedParticlesAny = -1;
    ...
    public boolean checkFinalState(PhysicsEvent event)
    {
        ...
        if(this.nChargedParticlesAny>=0){
            if(nChargedParticlesCount!=(pc+pm)) return false;
        } else {        
    ...
    public static int eventHash(PhysicsEvent event){
        int npositive = event.countByCharge(+1);
        int nnegative = event.countByCharge(-1);
        int nneutral  = event.countByCharge(0);
        if(npositive>=2)  npositive = 2;
        if(nnegative>=2)  nnegative = 2;
        if( nneutral>=2)   nneutral = 2;
    ...
    public void clear()
    {
        ...
        nChargedParticlesCount = 0;
        nChargedParticlesAny = -1;
    }
        ...
        if(pdgParticle!=null)
        {
            ...
            if(charge==0)
            {
                nNeutralParticlesWithPid += 1;
                    ...
                    nPositiveParticlesWithPid += 1; nChargedParticlesCount += 1;
                } else {
                    nNegativeParticlesWithPid += 1; nChargedParticlesCount += 1;
                }
    ...
    public void setInclusive(char charge)
    {
        switch (charge)
        {
            case '+': nPositiveParticlesAny = -1; nChargedParticlesAny = -1;
                break;
            case '-': nNegativeParticlesAny = -1; nChargedParticlesAny = -1;
                break;
            case 'n': nNeutralParticlesAny  = -1;
                break;
            case 'c': nChargedParticlesAny = -1;
                break;
            default: break;
        }
    }
    ...
    public void increaseChargeString(char charge, int weight)
    {
        switch (charge)
        {
            case '+': nPositiveParticlesCount += weight; nChargedParticlesCount += weight;//increaseCharge(1,weight);
                break;
            case '-': nNegativeParticlesCount += weight; nChargedParticlesCount += weight;//increaseCharge(-1,weight);
                break;
            case 'n': nNeutralParticlesCount  += weight;//increaseCharge(0,weight);
                break;
            case 'c': nChargedParticlesCount += weight; nChargedParticlesAny = 0;
                break;
            default: break;
        }
    }
    ...
    {
        switch (charge){
            case 1: if(nPositiveParticles>=0){ nPositiveParticles+=weight; nChargedParticlesCount += weight;}
            break;
            case -1: if(nNegativeParticles>=0){ nNegativeParticles+=weight; nChargedParticlesCount += weight;}
            break;
    ...
    public final void setFilter(String opt)
    {
        ...
        for(int loop = 0; loop < tokens.length; loop++)
        {
            ...
            if(tokenLen>0)
            {
                ...
                if(trailingChar=='+'||trailingChar=='-'||trailingChar=='n'||trailingChar=='c')
                {
    ...
    public String toString()
    {
        ...
        str.append(String.format("FILTER:---> POSITIVE : %6d %6d, NEGATIVE : %6d %6d , "
                + "NEUTRAL : %6d %6d NUMBER OF CHARGED TRACKS = %d ANY = %d",
                getMinimumByCharge( 1),getMaximumByCharge( 1),
                getMinimumByCharge(-1),getMaximumByCharge(-1),
                getMinimumByCharge( 0),getMaximumByCharge( 0), this.nChargedParticlesCount,
                this.nChargedParticlesAny));
