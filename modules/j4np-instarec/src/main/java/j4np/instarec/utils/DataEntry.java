...
public class DataEntry {
    ...
    public double distance(DataEntry entry){
        double distance = 0.0;
        float[] firstsrc = this.features();
        float[] firstdst = entry.features();
        for(int i = 0; i < firstsrc.length; i++){
            distance += Math.sqrt((firstsrc[i]-firstdst[i])*(firstsrc[i]-firstdst[i]));
        }
        return distance;
    }
