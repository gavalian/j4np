...
public class DeepNettsTrainer {
    ...
    public DataSet convert(DataList list, EntryTransformer transformer){
        ...
        String[] names = DeepNettsTrainer.generateNames(nInputs, nOutputs);
    ...
    public static FeedForwardNetwork create(int[] layers, ActivationType hidden, 
            ActivationType output, LossType lossFunction){
         FeedForwardNetwork.Builder b = FeedForwardNetwork.builder();        
        b.addInputLayer(layers[0]);
        
        for(int i = 1; i < layers.length-1; i++){
            b.addFullyConnectedLayer(layers[i], hidden);            
        }        
        b.addOutputLayer(layers[layers.length-1], output)
                .lossFunction(lossFunction);
                //.randomSeed(456);
        
        FeedForwardNetwork network = b.build(); 
        return network;
    }
    public static FeedForwardNetwork createClassifier(int[] layers){
    ...
    public static void main(String[] args){
        ...
        DataList data = DataList.csv("sample.csv", new int[]{0,1,2,3,4,5}, new int[]{6,7,8});
        
        FeedForwardNetwork     network = DeepNettsIO.read(previous);
