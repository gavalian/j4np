...
public class Track {
    ...
    public double[] features(){
        ...
        f[0] = vector.mag()/10.0;
        f[1] = (Math.toDegrees(vector.theta()) - 5)/40.0;
        f[2] = (vector.phi() + Math.PI)/(2*Math.PI);
        f[3] = (vertex.z() + 15.0)/20.0;
        return f;
    }
    ...
    public static  int countSector(List<Track> trk, int sector){
        int counter = 0;
        for(int i = 0; i < trk.size();i++) 
            if(trk.get(i).sector==sector)counter++;
        return counter;
    }
    ...
    public int matchSegments(Track trk){
        int counter = 0;
        for(int i = 0; i < this.means.length; i++){
            if(this.clusters[i]>0)
                if(Math.abs(this.means[i]-trk.means[i])<1.5) counter++;
        }
        return counter;
    }
    ...
    public static List<Track> filter(List<Track> list, TrackSelector sel){
        List<Track> selected = new ArrayList<>();
        
        for(int i = 0; i < list.size(); i++){
           if(sel.select(list.get(i))==true) selected.add(list.get(i));
        }
        return selected;
    }
