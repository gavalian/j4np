...
%results from processed data where the conventional algorithm has already 
%identified good tracks (with a cut on $\chi^2$ to select very good quality tracks), 
...
%of segments (6 segments forming a track candidate) is chosen that was not identified as a track by conventional algorithm.
...
To train the corruption auto-encoder network the same sample used for the classifier training was used.
 % good track candidates reconstructed by the conventional tracking algorithm are used (same sample 
 %for track classifier network training). 
